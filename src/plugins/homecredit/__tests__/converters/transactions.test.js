...
describe('"Bank Home Credit" convertTransaction', () => {
  const debetCardData = {
    account: {
      balance: 36050,
      id: '7005001234',
  ...
  it('should convert Debet Card transactions', () => {
    const transactionsIn = []
    const transactionsOut = []
    debetCardTransactions.forEach(value => {
      transactionsIn.push(value.in)
      transactionsOut.push(value.out)
    })
    const transactions = convertTransactions(debetCardData, transactionsIn)
    expect(transactions).toEqual(transactionsOut)
  })
})
