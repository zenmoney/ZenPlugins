...
### Запуск отладчика
Для запуска сервера разработки необходимо указать путь до папки с манифестом плагина в переменную окружения `PLUGIN` и в корне репозитория выполнить команду `npm run start`.
